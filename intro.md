...
The result is a **confident humility** of sorts. The theories that science upholds at each point in time are _justified_ in that one can reasonably argue for them providing the best current explanation given logic and observational data. At the same time, these theories are not held dear or close to the heart. They are easily disbanded, if new observations (new data) casts them into question. We thus can think of scientific theories as:
    strongly supported views that are only loosely held on to.
